...
def extract_red_flags(text):
    ...
    financial_patterns = [
        ...
        (r"internship\s+fee", "Charges internship fee - SCAM INDICATOR"),
        (r"enrollment\s+fee", "Requires enrollment fee - SCAM INDICATOR"),
        (r"one.time\s+(fee|charge|payment)", "One-time fee required - SCAM INDICATOR"),
        (r"confirm\s+your\s+seat", "Seat confirmation fee tactic"),
        (r"starter\s+kit", "Requests payment for starter kit"),
        (r"training\s+(materials\s+)?fee", "Charges for training materials"),
        (
            r"purchase.{0,20}(laptop|computer|equipment)",
    ...
    urgency_patterns = [
        ...
        (r"confirm\s+your\s+(seat|enrollment)", "Pressures to confirm seat"),
        (r"click\s+here\s+to\s+register", "Aggressive registration push"),
        (r"complete\s+enrollment", "Forced enrollment tactic"),
    ]
...
def calculate_risk_score(ml_confidence, red_flags):
    ...
    # If ANY critical flag found, minimum score is 0.75 (FAKE)
    if critical_count >= 1:
        adjusted_confidence = max(adjusted_confidence, 0.75)

    # More critical flags = higher confidence
    if critical_count >= 3:
        adjusted_confidence = max(adjusted_confidence, 0.95)
    elif critical_count >= 2:
        adjusted_confidence = max(adjusted_confidence, 0.88)

    # Total flag count also matters
    if total_flags >= 8:
        adjusted_confidence = max(adjusted_confidence, 0.92)
    elif total_flags >= 5:
...
def predict():
    ...
    try:
        ...
        response = {
            ...
            "risk_adjustment": (
                f"Risk score adjusted from {ml_confidence*100:.1f}% to {adjusted_confidence*100:.1f}% based on {len(red_flags_detected)} red flags"
                if abs(adjusted_confidence - ml_confidence) > 0.01
                else None
            ),
        }
...
def batch_predict():
    ...
    try:
        ...
        if not data or "job_descriptions" not in data:
            return (
                jsonify({"error": "Missing 'job_descriptions' array in request body"}),
                400,
            )
        ...
        for idx, job_desc in enumerate(job_descriptions):
            try:
                ...
                if not cleaned_text:
                    results.append(
                        {"index": idx, "error": "No valid text after preprocessing"}
                    )
                    continue
